...
public class LList<T> implements ListInterface<T> {
    ...
    public LList(LList<T> old) {
        firstNode = new Node(old.firstNode.getData(), old.firstNode.getNextNode());
        numberOfEntries = old.numberOfEntries;
        Node currNode = firstNode;
        while (currNode.getNextNode() != null) {
            Node nextNode = new Node(currNode.getNextNode().getData(), currNode.getNextNode().getNextNode());
    ...
    public boolean equals(LList<T> rhs) {
        if (numberOfEntries != rhs.numberOfEntries)
            return false;
        Node currNode1 = firstNode, currNode2 = rhs.firstNode;
        while (currNode1 != null) {
            if (!currNode1.getData().equals(currNode2.getData()))
                return false;
