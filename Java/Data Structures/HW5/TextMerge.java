...
public class TextMerge {
	...
	public static <T extends Comparable<? super T>> void mergeSortB(T[] a, int n) {
		mergeSortB(a, 0, n - 1);
	} // end mergeSortB

	private static <T extends Comparable<? super T>> void mergeSortB(T[] a, int first, int last) {
		if (first < last) { // sort each half
			int mid = (first + last) / 2;// index of midpoint
	...
	private static <T extends Comparable<? super T>> void mergeB(T[] a, int first, int mid, int last) {
		// Two adjacent subarrays are a[beginHalf1..endHalf1] and
		// a[beginHalf2..endHalf2].
		int beginHalf1 = first;
		int endHalf1 = mid;
	...
	public static void main(String[] args) {
		...
		System.out.printf("Enther the array size: ");
		int n = io.nextInt();
		...
		System.out.println("Fill the array with random number between 0 and 1000000.");
		long aTimeBegin = System.nanoTime();
		mergeSort(a, n);
		long aTimeEnd = System.nanoTime();

		System.out.printf("MergeSort() uses %.5fs.\n", (double) (aTimeEnd - aTimeBegin) / 1e9);

		long bTimeBegin = System.nanoTime();
		mergeSortB(b, n);
		long bTimeEnd = System.nanoTime();

		System.out.printf("MergeSortB() uses %.5fs.\n", (double) (bTimeEnd - bTimeBegin) / 1e9);
